...
let init = () => {
    ...
    routerElement?.addEventListener("route-changed", event => {
        ...
        let siteTitle = "";
        let headerElement = document.querySelector("wpvs-header") as HTMLElement | null;
        if (headerElement && headerElement.dataset.siteTitle) siteTitle = headerElement.dataset.siteTitle;

        let pageTitle = "";
        let pageElement = (detail.sRoot as ShadowRoot).querySelector("wpvs-page") as HTMLElement | null;
