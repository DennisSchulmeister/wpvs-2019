 ...
 *   * "": No linebreaks found
 */
function determineLinebreaks(text: string): string {
    if (text.includes("\r\n")) return "\r\n";
 ...
 */
function shiftLinesLeft(text: string): string {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        const whitespace: string = match ? match[0] : "";
 ...
 */
function removeLeadingLinebreaks(text: string): string {
    let linebreak = determineLinebreaks(text);
    if (linebreak === "") return text;
 ...
 */
function removeTrailingLinebreaks(text: string): string {
    let linebreak = determineLinebreaks(text);
    if (linebreak === "") return text;
